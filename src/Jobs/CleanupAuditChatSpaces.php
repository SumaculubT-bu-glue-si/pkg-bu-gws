...
class CleanupAuditChatSpaces implements ShouldQueue
{
    ...
    {
        try {
            Log::info('Starting audit chat spaces cleanup');

            // Find audit plans that are overdue and have chat spaces
            $overdueAudits = AuditPlan::whereNotNull('chat_space_id')
                ->where('due_date', '<', Carbon::now())
                ->where('chat_space_cleanup_scheduled', false)
                ->get();

            Log::info('Found overdue audits with chat spaces', [
                'count' => $overdueAudits->count()
            ]);
            ...
            Log::info('Audit chat spaces cleanup completed', [
                'deleted_spaces' => $deletedSpaces,
                'failed_deletions' => $failedDeletions
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to cleanup audit chat spaces', [
                'error' => $e->getMessage()
            ]);
            throw $e;
        }
